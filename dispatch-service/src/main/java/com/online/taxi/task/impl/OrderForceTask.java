        ...
        try {
            ...
            for (Integer distance : taskCondition.getDistanceList()) {
                ...
                for (DriverData driverData : list) {
                    log.info("#orderId= " + orderId + "  round = " + round + "司机信息：" + JSONObject.fromObject(driverData));
                    ...
                    String redisKey = Const.REDIS_KEY_DRIVER + driverData.getDriverInfo().getId();
                    log.info("#orderId= " + orderId + "  round = " + round + "车辆高德信息：" + JSONObject.fromObject(driverData.getAmapVehicle()));
                    try {
                        RedisLock.ins().lock(redisKey);
                        //前后1.5小时有单
                        int count = DispatchService.ins().countDriverOrder(driverData.getDriverInfo().getId(), startTime, endTime);
                        if (count > 0) {
                            //有单，不符合条件
                            continue;
                        }
                        ...
                        try {
                            if (StringUtils.isNotEmpty(order.getPassengerPhone())) {
                                bindAxbResponse1 = DispatchService.ins().bindAxb(EncriptUtil.decryptionPhoneNumber(driverData.getDriverInfo().getPhoneNumber()), EncriptUtil.decryptionPhoneNumber(order.getPassengerPhone()), order.getOrderStartTime().getTime() + TimeUnit.DAYS.toMillis(1));
                            ...
                            if (StringUtils.isNotEmpty(order.getOtherPhone())) {
                                bindAxbResponse2 = DispatchService.ins().bindAxb(EncriptUtil.decryptionPhoneNumber(driverData.getDriverInfo().getPhoneNumber()), EncriptUtil.decryptionPhoneNumber(order.getOtherPhone()), order.getOrderStartTime().getTime() + TimeUnit.DAYS.toMillis(1));
                        ...
                        updateOrder.setPlateNumber(driverData.getCarInfo().getPlateNumber());
                        updateOrder.setDriverId(driverData.getDriverInfo().getId());
                        updateOrder.setDriverPhone(driverData.getDriverInfo().getPhoneNumber());
                        updateOrder.setStatus(Const.ORDER_STATUS_DRIVER_ACCEPT);
                        updateOrder.setDriverStatus(Const.ORDER_DRIVER_STATUS_ACCEPT);
                        updateOrder.setCarId(Integer.parseInt(driverData.getAmapVehicle().getVehicleId()));
                        updateOrder.setIsFollowing(driverData.getIsFollowing());
                        ...
                        if (success) {
                            ...
                            //推送
                            String driverPhone = EncriptUtil.decryptionPhoneNumber(driverData.getDriverInfo().getPhoneNumber());
                            ...
                            pushDriver(order, orderRulePrice, driverData, passengerPhone);
                            pushPassanger(order, orderRulePrice, driverData, driverPhone);
                            //短信司机
                            smsDriver(order, orderRulePrice, driverData, passengerPhone, driverPhone);
                            pushOther(order, orderRulePrice, driverData, StringUtils.isEmpty(otherMappingNumber) ? driverPhone : otherMappingNumber);
                            pushPassenger(order, orderRulePrice, driverData, StringUtils.isEmpty(mappingNumber) ? driverPhone : mappingNumber);
                            return true;
                        } else {
